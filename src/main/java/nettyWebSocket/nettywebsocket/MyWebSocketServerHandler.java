 ...
 * 2、每个成功的线程访问顺序：channelActive（开启连接）-handleHttpRequest（http握手处理）
 * -messageReceived（消息接收处理）-handlerWebSocketFrame（实际处理，可以放到其他类里面分业务进行）
 * <p>
 * 3、注意：这个demo中我做了路由功能，在handleHttpRequest中对每个channel连接的时候对每个连接的url进行绑定参数，
 * 然后在messageReceived中获取绑定的参数进行分发处理（handlerWebSocketFrame或handlerWebSocketFrame2），同时也获取了uri后置参数，有注释。
 * <p>
 * 4、针对第三点路由分发，还有一种方法就是handshaker的uri（）方法，看源码即可，简单好用。
 * 5、群发的时候遍历集合或者map的时候，必须每个channle都实例化一个TextWebSocketFrame对象，否则会报错或者发不出。
 * <p>
 * （1）以上就是netty-websocket的Demo了，应该已经解释的很详细了，
 * 同时应对的并发量也满足一般企业用于websocket的连接，如果需要不够，可以用nginx负载均衡增加。
 * （2）最后给大家一条建议，在实际项目中，别让这种长连接一直保持，
 * 在nginx中可以设置连接无交流超时断开，大概设置10分钟左右，然后每8分钟定时从服务端发送一条心跳，具体想法就看你们喽~
 */
public class MyWebSocketServerHandler extends SimpleChannelInboundHandler<Object> {
    ...
    protected void messageReceived(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        // WebSocket接入
            ...
            System.out.println("请求地址："+handshaker.uri());
